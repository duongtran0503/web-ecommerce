...
const connector = connect((state) => ({
  product: state.data.keyandmouse,
}));
let i = 0;
const ShowKeyanhmouse = ({ product }) => {
  let list = [];
  for (let i = 0; i < 10; i++) {
    if (product[i]) {
      list.push(product[i]);
      ...
      (element, index) =>
       ...
       <div class="card-cpc">
         <div class="card-image-cpc">
          <img src="./image/product/keyandmouse/${element.image}" alt=""/>
...
export default connector(ShowKeyanhmouse);
